...
export default class Sprite11 extends Sprite {
  ...
  *whenIReceiveShop() {
    ...
    while (true) {
      if (
        this.stage.vars.Highscore == 40 ||
        this.stage.vars.Highscore > 40 ||
        this.stage.vars.OfSkinsUnlocked == 2 ||
          this.stage.vars.OfSkinsUnlocked > 2
      ) {
        if (this.stage.vars.OfSkinsUnlocked < 2) {
          this.stage.vars.OfSkinsUnlocked = 2;
        }
        if (this.stage.vars.Skin == 3) {
  ...
  *whenthisspriteclicked() {
    if (
      this.stage.vars.OfSkinsUnlocked == 2 ||
      this.stage.vars.OfSkinsUnlocked > 2 ||
      this.stage.vars.Highscore == 40 || this.stage.vars.Highscore > 40
    ) {
      this.costume = "costume1";
      this.stage.vars.Skin = 3;
    } else {
      null;
    }
  }
